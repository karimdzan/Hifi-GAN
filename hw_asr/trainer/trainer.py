...
class Trainer(BaseTrainer):
    ...
    def _log_predictions(
        ...
        shuffle(argmax_inds)
        argmax_inds_to_log = argmax_inds[:examples_to_log]
        argmax_texts_raw = [self.text_encoder.decode(inds) for inds in argmax_inds_to_log]
        argmax_texts = [self.text_encoder.ctc_decode(inds) for inds in argmax_inds_to_log]
        
        probs = log_probs.detach().cpu().numpy()
        lens = log_probs_length.detach().numpy()
        beam_search_texts = [
                            self.text_encoder.ctc_beam_search(probs[i][:lens[i]], 3)[0].text
